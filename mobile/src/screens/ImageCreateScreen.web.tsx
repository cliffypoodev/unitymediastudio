...
export function ImageCreateScreen() {
  ...
  const loadImageHistory = async () => {
    try {
      ...
      if (stored) {
        ...
        // Only load non-base64 images (URLs) to avoid storage issues, but allow data URLs on web
        const validHistory = history.filter(img =>
  ...
  const saveImageHistory = useCallback(async () => {
    try {
      // Save all images including data URLs on web
    ...
    try {
      ...
      switch (selectedModel) {
        ...
        case "nanobananapro":
          ...
          break;
        case "kling":
          // Kling not supported on web due to complex auth
          throw new Error("Kling AI is not available on web. Please use OpenAI or Google models.");
        default:
        ...
        >
          <View className="px-5 pt-4">
            ...
            {uniquePrompts.length > 0 && (
              <View className="mb-5">
                <Pressable
                  onPress={() => setShowPromptHistory(!showPromptHistory)}
            ...
            <View className="mb-5">
              ...
              <View className="flex-row">
                {aspectRatios.map((ratio, index) => (
                  <Pressable
                    key={ratio.value}
                    onPress={() => setSelectedAspect(ratio.value)}
                    disabled={isGenerating}
            ...
            <View className="mb-5">
              ...
              <View className="flex-row mb-2">
                ...
                <Pressable
                  onPress={() => setSelectedModel("google")}
                  disabled={isGenerating}
                  className="flex-1 py-3 rounded-xl items-center"
                  style={{
              ...
              {/* Row 2: Nano Banana Pro */}
              <View className="flex-row">
                <Pressable
                  onPress={() => setSelectedModel("nanobananapro")}
                  disabled={isGenerating}
                  className="flex-1 py-3 rounded-xl items-center"
                  style={{
              ...
              <Text className="text-xs mt-2" style={{ color: TWILIGHT.wolf }}>
                Note: Kling AI is only available on the mobile app
              </Text>
            </View>
          ...
          extraButtons={[
            {
              ...
              onPress: () => window.open("https://chat.openai.com", "_blank"),
              label: "Open ChatGPT"
            },
            {
              icon: "diamond-outline",
              onPress: () => window.open("https://gemini.google.com", "_blank"),
              label: "Open Gemini"
            }
          ]}
        />
